...
完成後，你能以台灣時區篩選今日 RSS、去除重複項目，並產生 JSON 與 Markdown 報告。

## 執行前準備

需要網路連線；本章不啟動 Chromium，但會受 RSS 來源、日期格式與搜尋服務限制影響。
...
成功時終端機會列出來源與標題，最後顯示類似：

```text
共 268 則，輸出 3 則。
結果：.../ai_news_2026-09-17.md
```

新聞數量會隨日期變動；檢查 JSON 的 `date`、`timezone`、`errors` 與 `articles`，不要用固定筆數判斷成功。
